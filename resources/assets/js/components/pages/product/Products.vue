    ...
    <section class="card">
      ...
      <div class="products-header">
          ...
          <div class="navigation-wrapper">
            <ul class="products-tab">
              <li
                ...
                class="products-tab-item clickable"
                :class="[tab == 'drug' ? 'active' : '']"
              >
                Show Drug Pricing
              </li>
              <li
              ...
              <li class="products-tab-item">
                ...
                >
                  ...
                  <option v-for="client in clients" :value="client.ClientID">
                    {{ client.CompanyName }}
                  </option>
                </select>
              </li>
              ...
              <li
                style="display: flex; align-items: center"
                class="products-tab-item"
              >
                {{ tab == 'drug' ? products.length : countries.length }} items
                found
              </li>
            </ul>
          </div>
        ...
        <div class="products-letter-filter mt-10 mb-10">
          <button
            @click="filter.letter = 'all'"
            class="products-button ml-10"
            :class="[!filter.letter || filter.letter == 'all' ? 'active' : '']"
          >
            All
          </button>
          <button
            @click="filter.letter = 'number'"
            class="products-button ml-5"
            :class="[filter.letter == 'number' ? 'active' : '']"
          >
            0-9
          </button>
          <button
            @click="filter.letter = letter"
            class="products-button ml-5"
            :class="[filter.letter == letter ? 'active' : '']"
        ...
        <div id="products-table" class="products-body">
            ...
            >
              <tr>
                <th v-for="(value, key) in visibleColumns" :key="key">
                  <select
                    class="product-table-select"
                    v-if="value == 'Product Type'"
                    v-model="filter.productType"
                  >
                    <option value="all">Product Type (all)</option>
                    <option value="1">Product Type (Medicine)</option>
                    <option value="2">Product Type (Test Kit)</option>
                  </select>

                  <select
                    class="product-table-select"
                    v-else-if="value == 'Fridge'"
                    v-model="filter.fridge"
                  >
                    <option value="all">Fridge (all)</option>
                    <option value="1">Fridge (Yes)</option>
                    <option value="0">Fridge (No)</option>
                  </select>

                  <select
                    class="product-table-select"
                    v-else-if="value == 'Package'"
                    v-model="filter.package"
                  >
                    <option value="all">Package (all)</option>
                    <option value="0">Package (Single)</option>
                    <option value="1">Package (Package)</option>
                  </select>

                  <select
                    class="product-table-select"
                    v-else-if="value == 'Reclassification'"
                    v-model="filter.reclassification"
                  >
                    <option value="all">Reclassification (all)</option>
                    <option value="0">Reclassification (POM)</option>
                    <option value="1">Reclassification (P)</option>
                  </select>
                  ...
                  >
                    <div v-if="filter.name.status">
                      <form @submit.prevent="saveFilter('name')">
                        ...
                        />
                        <i
                          @click="saveFilter('name')"
                          class="fa fa-check ml-10"
                        ></i>
                        <i
                          @click="clearFilter('name')"
                          class="fa fa-close ml-10"
                        ></i>
                      </form>
                    </div>
                    <div v-else @click="toggleFilter('name')">
                      <span>{{ value }}</span>
                      <span v-if="filter.name.value != ''"
                        ><b>({{ filter.name.value }})</b></span
                      >
                  ...
                  <select
                    class="product-table-select"
                    v-else-if="value == 'Pouchable'"
                    v-model="filter.pouchable"
                  >
                    <option :value="'all'">Pouchable (All)</option>
                  ...
                  >
                    ...
                    <div v-else @click="toggleFilter('code')">
                      <span>{{ value }}</span>
                      <span v-if="filter.code.value != ''"
                        ><b>({{ filter.code.value }})</b></span
                      >
                      <i class="fa fa-edit ml-10"></i>
                    </div>
                  </div>
          ...
          >
            <thead class="primary-color text-white">
              <tr>
                <th
                  v-for="(value, key) in countries[0]"
                  :key="key"
                  v-if="!hiddenColumns.includes(key)"
                >
                  {{ key }}
                </th>
                <th>Options</th>
            ...
            <tbody>
              ...
              <tr v-for="value in countries" :key="value.ProductCodeID">
                ...
                <td>
                  <button
                    title="View delivery pricing"
                    @click="editCountry(value)"
                    class="btn btn-primary table-icon"
                  >
                    <i class="fa fa-info"></i>
                  </button>
      ...
      <div class="products-footer card-footer"></div>
    </section>
...
export default {
  ...
  data() {
    return {
      loading: false,
      hiddenColumns: ['ProductCodeID', 'Status', 'VAT'],
      products: [],
      countries: [],
      alphabet: [
        'A',
      ...
      columns: {
        ...
        delivery: ['Code', 'Name', 'Price', 'Options'],
      },
      filter: {
        letter: 'all',
        company: 0,
  ...
  mounted() {
    ...
    table.addEventListener('scroll', () => {
      if (
        Math.ceil(table.scrollTop + table.clientHeight) + 5 >=
          table.scrollHeight &&
    ...
    if (typeof this.$route.query.code != 'undefined') {
      if (this.$route.query.code != '') {
        axios
          .get(`/inventory/products/${this.$route.query.code}`)
          .then((response) => {
  ...
  computed: {
    ...
    csvUrl() {
      return this.tab == 'drug'
        ? `/inventory/products/csv${this.searchString}`
        : `/inventory/countries/csv${this.searchString}`;
    },
  },
  watch: {
    searchString() {
  ...
  methods: {
    getProducts() {
      this.loading = true;
      axios
        .get(`/inventory/products${this.searchString}`)
        .then((response) => {
    ...
    getCountries() {
      this.loading = true;
      axios
        .get(`/inventory/countries${this.searchString}`)
        .then((response) => {
