    ...
    public function handle()
    {
        ...
        while ($i <= $lastPrescriptionID) {
            ...
            if ($prescription) {
                ...
                $matchingPrescriptionsCount = DB::table('Prescription')->whereRaw(
                    "Name = ? AND Surname = ? AND DOB = ? AND Sex = ?",
                ...
                if ($matchingPrescriptionsCount > 0) {
                    $matchingPrescriptions = DB::table('Prescription')->whereRaw(
                        "Name = ? AND Surname = ? AND DOB = ? AND Sex = ?",
                        [$prescription->Name, $prescription->Surname, $prescription->DOB, $prescription->Sex]
                    )
                ...
                //search for partial
                $matchingPrescriptionsCount = DB::table('Prescription')->whereRaw(
                    "(Name = ? OR Surname = ?) AND DOB = ? AND Sex = ?",
                    [$prescription->Name, $prescription->Surname, $prescription->DOB, $prescription->Sex]
                )
                    ->where('PrescriptionID', '!=', $i)
                    ->where('CustomerID', 0)
