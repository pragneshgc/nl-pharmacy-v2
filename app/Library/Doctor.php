...
class Doctor
{
    ...
    public function getPharmacy()
    {
        return DB::table('Client AS cl')
            ->select(["cl.*", "c.Name AS CName", "c.CodeName2 AS CodeName"])
            ->leftJoin('Country AS c', 'c.CountryID', '=', 'cl.CountryID')
            ->where('cl.Type', 1)->first();

        //instead of this we fetch the pharmacy from the client table
        // return DB::table('Doctor AS d')->select(["d.*", "c.Name AS CName", "c.CodeName2 AS CodeName"])
