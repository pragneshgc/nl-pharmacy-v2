        ...
        <div class="prescriptionStats flex-center">
          ...
          <div class="list">
            <ul v-if="loaded">
              <li
                ...
                v-for="(value, key) in statistics.statistics"
                v-if="!roleVisibility[userInfo.role].includes(key)"
          ...
          <div v-if="loaded" class="total flex-align-center clickable">
            ...
            <a
              ...
              class="btn smallTextBtn secondaryBtn"
      ...
      <section>
        <div class="orderSearch">
          <data-table
            ...
            :checkbox-visible="
              orderFilter == 'new' && [29, 30, 35].includes(userInfo.role)
            "
            :hidden-columns="['checked', 'disabled', 'UPSAccessPointAddress']"
          ...
          >
              ...
              <div class="row check-row">
                <div class="button-group">
                  ...
                  >
                    ...
                    <input
                      :class="{ unchecked: !match && currentChecked }"
                      :checked="match || currentChecked"
                      type="checkbox"
                      name="checkall"
                    />
                  ...
                  <div class="btn btnSize02 secondaryBtn dropdown">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li
                        v-if="appInfo.delivery_companies.includes('dpd')"
                        @click="checkByProperty('delivery', 4)"
                        class="dropdown-menu-item"
                      >
                        Check DPD
                      </li>
                      <li
                        v-if="appInfo.delivery_companies.includes('dhl')"
                        @click="checkByProperty('delivery', 10)"
                        class="dropdown-menu-item"
                      >
                        Check DHL
                      </li>
                      <li
                        v-if="appInfo.delivery_companies.includes('ups')"
                        @click="checkByProperty('delivery', 7)"
                        class="dropdown-menu-item"
                      >
                        Check UPS
                      </li>
                      <li
                        v-if="appInfo.delivery_companies.includes('rml')"
                        @click="checkByProperty('delivery', 5)"
                        class="dropdown-menu-item"
                      >
                        Check RM
                      </li>
                      ...
                      <li
                        @click="checkByProperty('client', 50)"
                        class="dropdown-menu-item"
                      >
                        Check Treated
                      </li>
                      <li
                        @click="checkByProperty('client', 51)"
                        class="dropdown-menu-item"
                      >
                        Check EveAdam
                      </li>
                      <li
                        @click="checkByProperty('client', 49)"
                        class="dropdown-menu-item"
                      >
                        Check Apport Sarl
                      </li>
                ...
                <div class="check-options" v-if="checked.length > 0">
                  <button
                    ...
                    class="btn btnSize02 secondaryBtn"
                  >
                    <span v-if="userInfo.role == 20 || userInfo.role == 19"
                      >Take over</span
                    ><span v-else>Add To Tray</span> ({{ checked.length }}
                    orders)
                  </button>
                </div>
              </div>
            ...
            <template
              v-if="orderFilter == 'safety' && userInfo.role == 40"
              v-slot:tools="slotProps"
            >
              <a
                class="btn btn-primary waves-effect table-icon clickable"
                @click="safe(slotProps.id)"
              >
                Safe
              </a>
            </template>
          ...
          <div
            class="card prescription-pool"
            v-else-if="orderFilter == 'ordercount'"
          >
            <!-- <div class="box-array">
            ...
            <div class="card-body order-id-list">
              <ul>
                ...
                >
                  <b
                    >MISSING PHARMACY ORDERS
                    <span
                      class="badge"
                      :class="[pendingPharmacyOrders.length > 0 ? 'red' : '']"
                      >{{ pendingPharmacyOrders.length }}</span
                    ></b
                  >
                  <div>
                    <button
                    ...
                    >
                      <i
                        class="fa fa-refresh"
                        :class="[
                          loadingPendingPharmacy ? 'spin-animation' : '',
                        ]"
                      ></i>
                    </button>
                  </div>
                ...
                <li
                  class="pool-list-layout"
                  v-if="pendingPharmacyOrders.length > 0"
                >
                ...
                <li
                  class="pool-list-layout"
                  v-for="order in pendingPharmacyOrders"
              ...
              <ul
                v-if="pendingPharmacyOrdersCount + pendingPrescriberCount == 0"
              >
              ...
              <ul class="mt-10">
                ...
                >
                  <b
                    >POSTPONED SHIPPING ORDERS PENDING
                    <span
                      class="badge"
                      :class="[onHoldOrders.length > 0 ? 'red' : '']"
                      >{{ onHoldOrders.length }}</span
                    ></b
                  >
                </li>
              ...
              <ul class="mt-10">
                ...
                >
                  <b
                    ...
                    <span
                      ...
                      >{{ pendingOrderAlerts.length }}</span
                    ></b
                  >
                  <div>
                    <button
                ...
                <li
                  class="pool-list-layout"
                  v-if="pendingOrderAlerts.length > 0"
                >
                  <div class="pool-column"><b>Reference Number</b></div>
                ...
                <li
                  class="pool-list-layout"
                  v-for="alert in pendingOrderAlerts"
                  :key="alert.NoteID"
                >
                  <div class="pool-column">
                    <div v-if="alert.Subscription">
                      <small
                        >Subscription ID: <b>{{ alert.Subscription }}</b></small
                      >
                      <br />
...
export default {
  ...
  methods: {
    ...
    getCount(refresh = false) {
      ...
      if (refresh) {
        ...
        axios
          ...
          .then((response) => {
            this.pendingPrescriberCount =
              response.data.data.pendingPrescriberCount;
            this.pendingPharmacyOrders =
              response.data.data.pendingPharmacyOrders;
            this.pendingPharmacyOrdersCount =
              response.data.data.pendingPharmacyOrdersCount;
          })
          .catch((error) => {
            this.postError(error.response.data.message);
          })
    ...
    getPendingCSV() {
      ...
      this.exportCSV(
        pending,
        `Pending Pharmacy Orders ${new Date().toLocaleString()}`
      );
    },
    ...
    redirectCallback() {
      if (
        this.orderFilter == 'new' &&
        this.trayLength < 1 &&
        (this.userInfo.role == 30 ||
          this.userInfo.role == 35 ||
          this.userInfo.role == 29)
      ) {
        //disable for admins
        axios
          .post(`/tray/new/insert/20`)
          .then((response) => {
            this.postSuccess(response.data.message);
    ...
    addToTray() {
      axios
        .post('/tray', { PrescriptionID: this.checked })
        .then((response) => {
          this.postSuccess(response.data.message);
    ...
    safe(id) {
      axios
        .post(`/order-edit/${id}/status`, { status: 1 })
        .then((response) => {
          this.$root.$emit('table.refresh');
        })
        .catch((error) => {
    ...
    checkAll(limit = false) {
      axios
        .get(`/orders/ids?f=${this.orderFilter}&intray=false&limit=${limit}`)
        .then((response) => {
    ...
    checkByProperty(type, property) {
      axios
        .get(
          `/orders/ids?f=${this.orderFilter}&intray=false&type=${type}&property=${property}`
        )
        .then((response) => {
          this.$store.commit('replaceChecked', response.data.data);
        })
