...
class PrescriptionPool
{
    ...
    public function getOrdersDetailed($ids)
    {
        ...
        $data = DB::table('Prescription AS p')
            ...
            ->selectRaw("DATE_FORMAT(FROM_UNIXTIME(p.CreatedDate), '%e %b %Y %H:%i') AS 'Date Recieved'")
            ->selectRaw("DATE_FORMAT(FROM_UNIXTIME(p.UpdatedDate), '%e %b %Y %H:%i') AS 'Date Supplied'")
