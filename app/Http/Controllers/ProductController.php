...
class ProductController extends Controller
{
    ...
    public function csv(Request $request)
    {
        ...
        $columns = ['Code', 'Name', 'Quantity', 'Units', 'Product Type', 'Pack', 'Reclassification', 'Fridge', 'TariffCode', 'Price', 'VAT'];
            ...
            foreach ($data as $product) {
                if ($i == 0) {
                    fputcsv($file, [
                        ...
                        $product->{'Product Type'}, $product->Package, $product->Reclassification, $product->Fridge, $product->TariffCode,
                        $product->Price,
                        $product->VAT
                    ], ',');
                } else {
