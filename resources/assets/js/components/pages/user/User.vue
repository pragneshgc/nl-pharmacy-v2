    ...
    <div class="content">
        <section class="card">
            ...
            ">
                        ...
                        <div style="display: flex; flex-direction: column; width: 30%">
                            ...
                            <select v-if="userInfo.role >= 50" v-model="data.esa_user_id"
                                ...
                                <option v-for="user in esaUsers" :value="user.UserID">
                                    {{ user.Username }}
                                </option>
                            </select>

                            <div v-if="errors.esa_user_id" class="invalid-feedback d-block">
                                {{ errors.esa_user_id[0] }}
                            </div>
                        </div>

                        <div style="display: flex; flex-direction: column; width: 30%">
                            <p>
                                2FA is Currently <b v-if="passwordSecurityStatus">Enabled</b>
                                <b v-else>Disabled</b> for this account.
                            </p>
                            <div v-if="passwordSecurityStatus">
                                <div v-html="code"></div>
                                <p>
                                    This QR code can be scanned by an authenticator app to start
                                    using it.
                                </p>
                            </div>
                        </div>
            ...
            ">
                        ...
                        <div v-if="errors.inventory_role" class="invalid-feedback d-block">
                            {{ errors.role[0] }}
                        </div>
                        ...
                        <div v-if="errors.shipping_role" class="invalid-feedback d-block">
                            {{ errors.shipping_role[0] }}
                        </div>
                    </div>
            ...
            ">
                        ...
                        <div v-if="errors.default_app" class="invalid-feedback d-block">
                            {{ errors.default_app[0] }}
                        </div>
                    </div>
                    ...
                    <div>
                        <button @click="togglePasswordChange" class="btn btnSize01 secondaryBtn" type="button">
                            Change password
                        </button>
                        <!-- <button @click="toggle2FA" class="btn btnSize01 secondaryBtn" type="button">Enable 2FA</button> -->
                        <button @click="toggleEmailChange" class="btn btnSize01 secondaryBtn" type="button">
                            Change email
                        </button>
                        <button @click="toggleLoginCodeChange" class="btn btnSize01 secondaryBtn" type="button">
                            Change login code
                        </button>
...
export default {
    ...
    methods: {
        ...
        getPasswordSecurityStatus: function (id) {
            axios
                .get(`/users/${this.$route.params.id}/2fa-status`)
                .then((response) => {
                    this.passwordSecurityStatus = response.data.data;
                    if (this.passwordSecurityStatus) {
                        axios
                            .get(`/users/${this.$route.params.id}/2fa-code`)
                            .then((response) => {
        ...
        enable2fa: function () {
            axios
                .post(`/users/${this.$route.params.id}/2fa-enable`)
                .then(() => {
                    this.getPasswordSecurityStatus();
                })
                .catch((error) => {
        ...
        disable2fa: function () {
            axios
                .post(`/users/${this.$route.params.id}/2fa-disable`)
                .then(() => {
                    this.getPasswordSecurityStatus();
                })
                .catch((error) => {
