    ...
    <div>
        <div class="infoHeader" :class="headerClass">
            <div class="floatLeft" style="min-width: 402px">
                <ul>
                    ...
                    <li title="Double click to copy Client Reference Number to clipboard" class="clickable clipboard-copy"
                        @dblclick="copyToClipboard(prescription.ReferenceNumber)" style="min-width: 120px">
                            ...
                            <button title="Update prescription status to the selected option"
                                class="btn btnSize04 primaryBtn" v-if="fullyLoaded &&
                                    ((prescriptionStatus != prescription.Status &&
                                        prescription.SubStatus == null) ||
                                        (prescriptionStatus != prescription.SubStatus &&
                                            prescription.SubStatus != null))
        ...
        <transition name="fade">
            <div v-if="prescription" class="content prescription-content">
                <section v-if="duplicate &&
                    [2, 4, 5, 6, 7, 8, 9, 10, 11, 1].includes(prescription.Status)
                    " class="notranslate">
                    <div class="infoBox warning">
                        <p>
                            There is a possible duplicate order with ID
                            <a target="_blank" :href="`#/prescription/${duplicate.PrescriptionID}`">{{
                                duplicate.PrescriptionID }}</a>
                            that has the same customer reference id
                            {{ duplicate.ReferenceNumber }} with status
                            {{ orderStatuses[duplicate.Status]
                            }}{{ duplicate.SubStatus ? ' -
                            '+orderSubStatuses[duplicate.SubStatus] : ''
}}.<br />
                            Please investigate by
                            <a target="_blank" :href="`#/prescription/${duplicate.PrescriptionID}`">clicking here</a>
                            before processing.
                        </p>
                    </div>
                </section>
                ...
                <transition name="slide-down">
                    <section class="card" v-if="prescription.Status == 9">
                        <div class="card-header flex flex-space-between flex-center-vertically" :class="[
                            prescription.Message == 'Address Validated Successfully' ||
                                prescription.Message == ''
                                ? 'card-header-success'
                                : 'card-header-warning',
                        ]">
                            <h3>Safety Check</h3>
                            <button class="btn btnSize02 secondaryBtn" v-if="false">
                                View Blacklist (new window)
                            </button>
                            <div>
                                ...
                                <button :disabled="loadingValidation" v-if="prescription.Message != 'Address Validated Successfully'
                        ...
                        <div class="card-body">
                            <p v-if="prescription.Message != ''" style="font-size: 19px">
                                <b v-html="prescription.Message" />
                            </p>
                            <p v-else>Unknown issue</p>
                            <hr class="mt-10 mb-10" v-if="prescription.Notes != '' &&
                    ...
                    <div class="patientInfo card-body">
                        <div class="patient">
                            <ul>
                                ...
                                <li class="gender" :class="[
                                        prescription.Sex == 'Male'
                                            ? 'blue'
                                            : prescription.Sex == 'Female'
                                                ? 'purple'
                                                : prescription.Sex == 'Transgender'
                                                    ? 'orange'
                                                    : 'grey',
                                    ]">
                                    <span>Gender: </span>
                        ...
                        <div class="prescription-info">
                            <div class="prescription">
                                <ul>
                                    <li>
                                        <span>Recieved Date: </span>{{ timestampToDate(prescription.CreatedDate) }}
                                    </li>
                                    <li v-if="prescription.Status == 8">
                                        <span>Shipped/Supplied Date: </span>{{ timestampToDate(prescription.UpdatedDate) }}
                                    </li>
                                    <li v-if="isCommercial">
                                        <span>Commercial Invoice Value: </span>{{ prescription.Repeats }}
                                    </li>
                                    <li>
                                        <span>Shipping: </span>Patient has Authorised 3rd Party
                                        Carrier
                                    </li>
                                    <li><span>Courier: </span>{{ prescription.Courier }}</li>
                                    <li v-if="prescription.TrackingCode != '' &&
                                        prescription.TrackingCode != null
                                        ">
                                    ...
                                    <li v-else-if="(prescription.TrackingCode == '' ||
                                            prescription.TrackingCode == null) &&
                                        userInfo.role >= 40 &&
                                        prescription.Status == 7
                                        ">
                                        <span>Tracking Code: </span>{{ prescription.TrackingCode }}
                                        <button @click="addTracking()" class="clickable smallTextBtn secondaryBtn">
                            ...
                            <div class="location">
                                <ul>
                                    <li>
                                        <span>Home address: </span>{{ prescription.Address1 }}
                                        {{ prescription.Address2 }} {{ prescription.Address3 }}
                                        {{ prescription.Address4 }} {{ prescription.Postcode }}
                                    </li>
                                    <li>
                                        <span>Delivery address: </span>
                                        {{ prescription.DAddress1 }} {{ prescription.DAddress2 }}
                                    ...
                                    <li v-if="(prescription.Telephone != '' && prescription.Telephone) || (prescription.Mobile != '' && prescription.Mobile)">
                                        <span>Telephone: </span>
                                        <a :href="`tel:${prescription.Telephone
                                    ...
                                    <li v-if="prescription.Email != '' && prescription.Email">
                                        <span>Email: </span>
                                        <a :href="`mailto:${prescription.Email}`">{{
                                            prescription.Email
                                        }}</a>
                                    </li>
                                </ul>
                        ...
                        <div class="activity" v-bind:style="activityBackground(prescription)">
                                ...
                                <li class="activity-log-item" v-for="a in activity" :key="a.ActivityID">
                                    <div class="activity-action">
                                        ...
                                        <button v-if="(a.Type == 750 || a.Type == 751) && userInfo.role >= 50
                                            " class="clickable smallTextBtn secondaryBtn" :disabled="locked" @click="revert(a)">
                                            Revert
                                        </button>

                                        <button v-if="(a.Type == 900 || a.Type == 904) &&
                                            (userInfo.role >= 50 ||
                                                userInfo.role == 20 ||
                                                userInfo.role == 19)
                                            " class="clickable smallTextBtn secondaryBtn" :disabled="locked" @click="resendPouch(a)">
                                            Resend OCS
                                        </button>

                                        <button v-if="(a.Type == 750 || a.Type == 751) &&
                                            userInfo.role < 50 &&
                                            !a.FirstChange
                                        ...
                                        <button v-else-if="(a.Type == 750 || a.Type == 751) &&
                                            userInfo.role < 50 &&
                                            a.FirstChange
                                            " class="clickable smallTextBtn secondaryBtn" :disabled="locked" @click="revert(a)">
                                            Revert
                            ...
                            <ul class="activity-log-wrapper" v-else>
                                <li class="activity-log-item">No activity log found</li>
                            </ul>
                        </div>
                    </div>
                </section>
                    ...
                    <div class="card-header" style="display: flex; justify-content: space-between">
                        <h3 style="display: flex; justify-content: space-between">
                            ...
                            <button v-if="prescription.Status == 8" class="smallTextBtn clickable"
                                ...
                                {{ showAttachedProducts ? 'Hide' : 'Show' }} Batch No/Expiry
                                Date
                            </button>
                        </h3>
                        <div style="padding-right: 5px; padding-top: 2px">
                            Layout:
                            <button :class="{ active: view.products == 1 }" class="clickable smallTextBtn secondaryBtn"
                                @click="view.products = 1">
                                1
                            </button>
                            <button :class="{ active: view.products == 2 }" class="clickable smallTextBtn secondaryBtn"
                                @click="view.products = 2">
                                2
                            </button>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="pouch-notification" v-if="isJVM">Pouch Order</div>
                ...
                " class="infoBox warning thin-error" v-if="!product.CorrectName">
                                <div style="font-size: 20px">
                                    ...
                                    {{ prescription.CompanyName }} ({{ product.Description }})
                    ...
                    " class="btn btnSize01 secondaryBtn">
                            ...
                            <div class="medicineTitle">
                                <div class="fridge-notification" v-if="product.Fridge == 1 &&
                                    (userInfo.role == 20 || userInfo.role == 19)
                                    ">
                                    FRIDGE PRODUCT
                                </div>
                                <div v-if="view.products == '1'" class="title information" :class="[
                                    product.Fridge == 1 &&
                                        (userInfo.role == 20 || userInfo.role == 19)
                                        ? 'mt-20'
                                        : '',
                                    ...
                                    <div class="medicine-tooltips">
                                        ...
                                        <div class="medicineTooltip">
                                            ...
                                            <div class="value">{{ !product.CorrectUnitName ? product.Unit : product.Units }}
                                            </div>
                                        </div>
                                    ...
                                    <div v-if="prescription.Status == 8 &&
                      ...
                      ">
                                        ...
                                        <div v-for="(product, key) in getAttachedProductAttributes(
                                                product.ProductID
                                            )" class="medicineTooltip" :key="key" style="
                        display: inline-block;
                        margin-right: 5px;
                        margin-top: 5px;
                      ">
                                            <div style="font-size: 0.9rem">Pack #{{ key + 1 }}</div>
                                            <div style="font-size: 0.9rem">
                                                Expiry:
                                                <b>{{
                                                    product.FMDExpiryDate ? product.FMDExpiryDate : 'N/A'
                                                }}</b>
                                            </div>
                                            <div style="font-size: 0.9rem">
                                                Batch:
                                                <b>{{
                                                    product.FMDBatchID ? product.FMDBatchID : 'N/A'
                                                }}</b>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="information" v-if="view.products == '2'" :class="[
                                    product.Fridge == 1 &&
                                        (userInfo.role == 20 || userInfo.role == 19)
                                        ? 'mt-30'
                                        : '',
                                    ...
                                    <div style="text-transform: uppercase">
                                        MEDICINE NAME:
                                        <b>{{ product.Description }} ({{ product.Code }})</b>
                    ...
                    <div class="scrollingSuccessors">
                        ...
                        <div class="medicineHistory notranslate">
                            ...
                            <ul @dblclick="redirect(value.PrescriptionID)" v-if="!historyLoading" class="new"
                                v-for="(value, key) in history" :key="key" :title="`Order ${value.PrescriptionID} in status ${orderStatuses[value.Status]
                                    }. Double-click to open in new tab.`" :class="statusClass(value.Status)">
                                <li v-for="(product, k) in value.Products" class="medicine" :key="k">
                                    <span v-if="k == 0 &&
                                        [8, 6, 3, 4, 12, 13, 14, 15].includes(value.Status)
                                        ">
                            ...
                            <div v-if="historyLoading" class="dotloader loader-relative" style="">
                                Loading...
                            </div>
                            ...
                            <ul class="tabs">
                                ...
                                <li title="Relates communication with perscriber and notes sent with perscription"
                                    ...
                                    <a href="javascript:;">Queried Notes
                                        <span v-if="notes.correspondence.length + notes.information.length >
                                            0
                                            " class="badge red">{{
        notes.correspondence.length + notes.information.length
                                ...
                                <li title="Relates to the current order" :class="{ active: activeTab == 'order' }"
                                    ...
                                    <a href="javascript:;">Order Notes
                                        <span v-if="(notes.other.length ||
                                                (prescription.Notes != '' &&
                                                    prescription.Notes != null)) > 0
                                            " class="badge red">
                                            {{
                                                notes.other.length +
                                                (prescription.Notes != '' && prescription.Notes != null
                                                    ? 1
                                                    : 0)
                                            }}
                                        </span>
                            ...
                            <div class="content" v-if="activeTab == 'notes'">
                                <ul v-if="notes.critical.length > 0" class="critical">
                                    <li class="note" :class="[
                                        ...
                                        <div class="note-footer" v-if="userInfo.role >= 20 &&
                                            note.DeletedAt == null &&
                                            note.EditedAt == null
                                            ">
                                            <div>
                                        ...
                                        <div class="note-footer" v-else-if="userInfo.role >= 50 && note.DeletedAt != null">
                                            <b class="clickable" @click="showEditHistory(note.NoteID)"
                                                v-if="note.Edits.length > 0 && userInfo.role >= 50" style="color: #ff8944">
                                                <span v-if="showEditHistoryFor.includes(note.NoteID)">Hide Edit
                                                    History</span>
                                                <span v-else>Show Edit History</span>
                                            </b>
                                        ...
                                        <div v-if="showEditHistoryFor.includes(note.NoteID)" class="note-footer">
                                            <ul class="critical" style="width: 100%">
                                                <li class="note" v-for="edited in note.Edits" :key="edited.NoteID">
                                                    ...
                                                    <div class="note-footer">
                                                        <span>{{ edited.name }} {{ edited.surname }}</span><span>{{
                                                            edited.CreatedAt }}</span>
                                                    </div>
                            ...
                            <div class="content" v-if="activeTab == 'patient'">
                                ...
                                <ul v-if="notes.correspondence.length == 0 &&
                                    notes.information.length == 0
                                    ">
                            ...
                            <div class="content" v-if="activeTab == 'order'">
                                ...
                                <ul v-if="(prescription.Notes == '' || prescription.Notes == null) &&
                        ...
                        <div class="medicineQuestionnaire" :class="{ fullscreen: expandedQuestionnaire }">
                            <h2>
                                ...
                                <span v-if="questionnaire.length != 0" class="language-toggle"
                                    @click="translateQuestionnaire()">({{ languageText }})</span>
                            ...
                            <div v-else class="dotloader loader-relative" style="">
                                Loading...
                            </div>
                ...
                <section class="card" v-if="appInfo.registration_style == 'GB'">
                    <div class="card-header" style="background: #eff8f830">
                        <h3 style="text-align: center">
                            HR HEALTHCARE Pharmacy, Unit 18, Waters Meeting, Britannia Way,
                            Bolton BL2 2HH, United Kingdom
                        </h3>
                    </div>
                </section>
            </div>
...
export default {
    ...
    computed: {
        ...
        fullyLoaded() {
            return (
                !this.loading &&
                !this.questionnaireLoading /*&& !this.historyLoading*/ &&
                !this.notesLoading &&
                !this.locked
            );
        },
        ...
        headerClass() {
            return [1, 7].includes(this.prescription.Status)
                ? 'active'
                : [2, 8].includes(this.prescription.Status)
                    ? 'success'
                    : [4, 5, 9, 10, 11, 12, 13, 14, 15].includes(this.prescription.Status)
                        ? 'warning'
                        : [3, 6].includes(this.prescription.Status)
                            ? 'error'
                            : [16].includes(this.prescription.Status)
                                ? 'returned'
                                : '';
        },
        tray() {
            ...
            if (this.prescription) {
                ...
                this.prescription.Products.forEach((product) => {
                    if (product.Name != product.Description) {
                        this.alternativeNameCheck(
                            product,
                            this.prescription.ClientID,
                            (result) => {
                ...
                if (productCodes.includes('1131663') && productCodes.includes('CPCR')) {
                    ...
                    productCodes.includes('1136225') &&
                    productCodes.includes('CPCR')
                ) {
                    show = true;
                } else if (
                    productCodes.includes('1131663') &&
                    productCodes.includes('CGON') &&
                    productCodes.includes('2385607')
                ) {
                    show = true;
                }
    ...
    methods: {
        ...
        checkLock(callback = false, preflight = false) {
            axios
                .get('/logs/locked/' + (preflight ? this.orderID : this.currentOrderID))
                .then((response) => {
        ...
        alternativeNameCheck(product, client, callback) {
            axios
                .get(
                    `/inventory/products/alternative-name?code=${product.ProductCodeID
                    }&name=${encodeURI(product.Description)}&client=${client}`
                )
                .then((response) => {
        ...
        unlockOrder() {
            axios
                .post(`logs/unlock/${this.currentOrderID}`)
                .then((response) => {
                    this.locked = false;
                })
                .catch((error) => {
        ...
        takeOverOrder() {
            axios
                .post(`logs/takeover/${this.currentOrderID}`)
                .then((response) => {
                    this.locked = false;
                })
                .catch((error) => {
        ...
        getAttachedProductAttributes(productId) {
            let product = this.attachedProducts.filter(
        ...
        getTestKits() {
            axios
                .get('/order/' + this.currentOrderID + '/test-kits')
                .then((response) => {
        ...
        getOrderHistory() {
            ...
            axios
                .get('/order/' + this.currentOrderID + '/history')
                .then((response) => {
        ...
        getQuestionnaire() {
            ...
            axios
                .get('/questionnaire/' + this.currentOrderID)
                .then((response) => {
        ...
        getNotes() {
            ...
            axios
                .get(`/order/${this.currentOrderID}/notes`)
                .then((response) => {
                    ...
                    if (
                        ...
                        this.notes.alerts.forEach((alert) => {
                            if (alert.DeletedAt == null && alert.EditedAt == null) {
                                ...
                                if (alert.Type != type) {
                                    type = alert.Type;
                                    html += `<li class="note-header ${type == 1 ? 'note-header__danger' : ''
                                        }"><div>${type == 1 ? 'Patient Notes' : 'Order Notes'
                                        }</div></li>`;
                                }
                                ...
                                title="${alert.Type == 2
                                        ...
                                        : alert.Type == 1
                                            ? 'Patient Alert'
                                            : 'Order Alert'
                                    } created by ${alert.name} ${alert.surname}"
                                style="${alert.Type == 2
                                        ? 'border-left: 5px solid #32a36a;'
                                        : alert.Type == 1
        ...
        updateStatus(text = false) {
            ...
            axios
                .post(
                    '/order-edit/' + this.prescription.PrescriptionID + '/status',
                    postData
                )
                .then((response) => {
                    if (this.prescription.Status == 9 && this.prescriptionStatus == 1) {
                        ...
                        this.prescription.Status == 9 &&
                        this.prescriptionStatus == 6
                    ) {
                        localStorage.setItem('dashboard.orderFilter', 'safety');
                        this.$router.push({ name: 'in tray' });
                    } else if (
                        ['4', '12', '13', '14', '15'].includes(this.prescriptionStatus)
                    ) {
        ...
        updateStatusPopup() {
            ...
            if (
                ['102', '107', '106', '69', '634', '164'].includes(
                    this.prescriptionStatus
                )
            ) {
                this.$swal({
                    // this.prescriptionStatus == '107'
                    text:
                        this.prescriptionStatus == '102'
                            ? 'Enter expiry date (MM/YYYY)'
                            : this.prescriptionStatus == '107'
                                ? 'Enter shipping date (DD/MM/YYYY)'
                                : 'Enter other text',
                    inputPlaceholder:
                        this.prescriptionStatus == '102'
                            ? 'MM/YYYY'
                    ...
                    inputValidator: (value) => {
                        return new Promise((resolve) => {
                            if (this.prescriptionStatus == '102') {
                                ...
                                let re = new RegExp(
                                    `((1[0-2])|(0[1-9]))\/[${year[0]}-9][${year[1]}-9][${year[2]}-9][${year[3]}-9]$`
                                );
                                let errorMessage = `
        ...
        statusClass(status) {
            return [1, 7].includes(status)
                ? 'active'
                : [2, 8].includes(status)
                    ? 'success'
                    : [4, 5, 9, 10, 11, 12, 13, 14, 15].includes(status)
                        ? 'warning'
                        : [16].includes(status)
                            ? 'returned'
                            : [3, 6].includes(status)
        ...
        getActivity() {
            axios
                .get('/order/' + this.currentOrderID + '/activity')
                .then((response) => {
        ...
        checkOrderStatuses() {
            axios
                .get(`/order/${this.currentOrderID}/statuses`)
                .then((response) => {
        ...
        validateAddress() {
            ...
            axios
                .post(`/api/validate-address/${this.currentOrderID}`)
                .then((response) => {
                    this.postSuccess(response.data.message);
                })
                .catch((error) => {
        ...
        revert(activity, readOnly = false) {
            ...
            axios
                .get(`/order/${this.currentOrderID}/access-point`)
                .then((response) => {
                    (async () => {
                        ...
                        instance.$on('difftable.loaded', () => {
                            if (readOnly) {
                                ...
                                this.$swal({
                                    ...
                                    if (result.value) {
                                        axios
                                            .post(`/order-edit/revert/${activity.ActivityID}`)
                                            .then((response) => {
                                                this.postSuccess(response.data.message);
                                            })
                                            .catch((error) => {
        ...
        revealEmail() {
            axios
                .get(`/order/${this.currentOrderID}/email`)
                .then((response) => {
        ...
        activityBackground(prescription) {
            if (prescription.UPSAccessPointAddress != 0) {
                return {
                    background:
                        'url(images/logo/ups_access_point.jpg) 0 5px / 30px no-repeat!important',
                };
            } else if (prescription.PaymentMethod != 0) {
                return {
                    background:
                        'url(images/logo/ups_cod.jpg?) 0 5px / 30px no-repeat!important',
                };
            } else if (prescription.DeliveryID == 7) {
                return {
                    background: `url(${this.imgMap[prescription.DeliveryID]
                        }) 0 5px / 30px no-repeat!important`,
                };
            } else {
                return {
                    background: `url(${this.imgMap[prescription.DeliveryID]
                        }) 0px 0px / 45px no-repeat!important`,
                    paddingLeft: '50px',
                };
            }
        },
        ...
        addTracking() {
            this.$swal({
                ...
                if (result.value) {
                    ...
                    axios
                        .post(`/order/${this.currentOrderID}/add-tracking`, {
                            code: result.value,
                        })
                        .then((response) => {
                            this.postSuccess(response.data.message);
        ...
        resendTracking(id) {
            ...
            axios
                .post(`/order/${this.currentOrderID}/resend-tracking`)
                .then((response) => {
                    this.postSuccess(response.data.message);
                })
                .catch((error) => {
        ...
        resendPouch(a) {
            axios
                .post(`/jvm/${this.currentOrderID}/send`)
                .then((response) => {
                    this.postSuccess(response.data.message);
                    this.getActivity();
                })
