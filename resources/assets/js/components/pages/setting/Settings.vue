        ...
        <div class="settings">
          <ul class="tabs">
            ...
            <li
              v-if="userInfo.role >= 50"
              title="Dev Sites"
              :class="{ active: activeTab == 'devsites' }"
          ...
          <div class="content pxp-form" v-if="activeTab == 'settings'">
            <p class="mb-10">
              These settings affect how many records are shown per page, how
              many orders a dispenser can assign to themselves, etc.
            </p>
            <p class="mb-10">
              <b>These are global settings and affect all users!</b>
            </p>
            <hr class="mb-10" />
            ...
            <div class="form-column">
              ...
              >
                ...
                <input
                  :disabled="!types[setting.Type]?.enabled"
                  v-model="update[setting.SettingID]"
                  :type="types[setting.Type]?.type"
                  :name="setting.SettingID"
                  class="form-control mb-3"
                  :placeholder="setting.Name"
                />
                <div
                  v-if="errors[setting.Name]"
                  class="invalid-feedback d-block"
                >
                  {{ errors[setting.Name][0] }}
                </div>
              </div>
            </div>
          ...
          <div class="content pxp-form" v-if="activeTab == 'delivery'">
            <p class="mb-10">
              These settings affect the API connection with various delivery
              companies
            </p>
            <p class="mb-10">
              <b
                >These are global settings and affect the entire application!</b
              >
            </p>
            <hr class="mb-10" />
            ...
            <div
              class="form-group form-group-2"
              v-for="setting in deliveryOptions"
              :key="setting.SettingID"
            >
              <label :for="setting.SettingID">{{ setting.Name }}</label>
              <input
                ...
                :placeholder="setting.Name"
              />
              <div v-if="errors[setting.Name]" class="invalid-feedback d-block">
                {{ errors[setting.Name][0] }}
              </div>
            </div>
          ...
          <div
            class="content pxp-form pxp-form-long"
            v-if="activeTab == 'details'"
          >
            <div class="infoBox warning mb-10">
              THIS IS THE MAIN COMPANY DETAIL. IF YOU MAKE CHANGES HERE IT WILL
              AFFECT THE SYSTEM I.E. DELIVERY NOTE
            </div>

            <div
              class="form-group form-group-2"
              style="width: 50%"
              v-for="(setting, key) in companySettingsUpdate"
              :key="key"
              v-if="!hiddenFields.includes(key)"
            >
              <label :for="key">{{ key }}</label>
              <input
                v-model="companySettingsUpdate[key]"
                :name="key"
                class="form-control mb-3"
                :placeholder="key"
              />
          ...
          <div class="content" v-if="activeTab == 'import'">
            ...
            <section
              class="text-center infoBox success mt-10"
              v-if="status != ''"
              v-html="status"
            />
            ...
            <section class="text-center">
              <button
                ...
                class="btn btnSize01 tertiaryBtn bigButton mt-10"
                @click="upload"
              >
          ...
          <div class="content pxp-form" v-if="activeTab == 'application'">
            <p class="mb-10">
              Make sure the name of the printer is same as reported by system.
              If not the application will use the systems default printer.
            </p>
            <p class="mb-10">
              <b
                >These settings only affect the local PC, and any user using the
                local PC</b
              >
            </p>
            <hr class="mb-10" />
            ...
            <div class="form-column">
              <div class="form-group form-group-2" v-if="!printAppOnline">
                ...
                <input
                  v-model="applicationSettings.labelPrinter"
                  type="text"
                  id="labelPrinter"
                  class="form-control mb-3"
              ...
              <div class="form-group form-group-2" v-else>
                ...
                >
                  ...
                  <option
                    v-for="(value, key) in printAppOnline"
                    :key="key"
                    :value="value"
                  >
                    {{ value }}
              ...
              <div class="form-group form-group-2" v-if="!printAppOnline">
                ...
                <input
                  v-model="applicationSettings.deliveryNotePrinter"
                  type="text"
              ...
              <div class="form-group form-group-2">
                <button
                  class="btn btnSize02 secondaryBtn"
                  @click="updateApplicationSettings()"
                >
                  Update
                </button>
              </div>
          ...
          <div class="content pxp-form" v-if="activeTab == 'downloads'">
            ...
            <ul class="list-group">
              <li class="list-group-item">
                <p>
                  Blank Pathology Form (<a target="_blank" href="/download/form"
                    >download</a
                  >/<a target="_blank" href="/view/form">view</a>)
                </p>
              </li>
              <!-- <li class="list-group-item">
          ...
          <div class="content pxp-form" v-if="activeTab == 'devsites'">
            <p class="mb-10">Below is a list of all active dev sites</p>
            <hr />
            <br />
            <ul
              class="list-group"
              v-if="
                location == 'https://esasys.co.uk' ||
                location == 'https://pharmacist.4sm-dev.xyz'
              "
            >
              <li class="list-group-item">
                <a href="https://old.esasys.co.uk"
                  >old.esasys.co.uk - Old ESA</a
                >
              </li>
              <li class="list-group-item">
                <a href="https://dev.esasys.co.uk"
                  >dev.esasys.co.uk - Old ESA Dev</a
                >
              </li>
              <li class="list-group-item">
                <a href="https://pharmacist.4sm-dev.xyz/"
                  >pharmacist.4sm-dev.xyz - ESA Dev</a
                >
              </li>
              <li class="list-group-item">
                <a href="https://inventory.4sm-dev.xyz"
                  >inventory.4sm-dev.xyz - Inventory Dev</a
                >
              </li>
              <li class="list-group-item">
                <a href="https://pxp.4sm-dev.xyz">pxp.4sm-dev.xyz - PXP Dev</a>
              </li>
              <!-- <li class="list-group-item">
                                <p>Blank Prescription (<a target="_blank" href="/download/prescription">download</a>/<a target="_blank" href="/view/prescription">view</a>)</p>
                            </li> -->
            ...
            <ul class="list-group" v-else>
              <li class="list-group-item">
                <a href="https://dev.pharmasys.xyz"
                  >dev.pharmasys.xyz - ESA Dev</a
                >
              </li>
              <li class="list-group-item">
                <a href="https://inventory.dev.pharmasys.xyz"
                  >inventory.dev.pharmasys.xyz - Inventory Dev</a
                >
              </li>
              <li class="list-group-item">
                <a href="https://shipping.dev.pharmasys.xyz"
                  >shipping.dev.pharmasys.xyz - Shipping Dev</a
                >
              </li>
            </ul>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      settings: [],
      companySettings: [],
      applicationSettings: localStorage.getItem('settings.application')
  ...
  methods: {
    ...
    getCompanySettings() {
      axios
        ...
        .then((response) => {
          ...
          this.companySettingsUpdate = JSON.parse(
    ...
    updateCompanySettings() {
      this.$swal({
        ...
        if (result.value) {
          axios
            .patch('/settings/company', this.companySettingsUpdate)
            .then((response) => {
