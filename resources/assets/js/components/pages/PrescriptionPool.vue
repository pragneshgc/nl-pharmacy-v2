  ...
  <div class="content prescription-pool">
    <section class="card">
      ...
      >
        <div>
          <button
            class="btn btnSize02 secondaryBtn mb-10"
          ...
          <button
            v-if="tray.length > 0"
            :title="'Release orders assigned to ' + userInfo.name"
            @click="release(userInfo.esa_user_id, false)"
          ...
          <button
            ...
            :disabled="count.available == orders.length"
            @click="release(false, false, true)"
          >
            Release All
          </button>
        </div>
        <transition name="fade">
          <div
            class="prescription-pool_print-log"
            v-if="printLog.length > 0 && showLogs"
          >
            <ul>
              ...
              <li v-for="log in printLog" :key="log.PrescriptionID">
                <span>
                  ...
                  <b>
                    <a
                      target="_blank"
                      :href="`#/prescription/${log.PrescriptionID}`"
                ...
                <button
                  ...
                  @click="reprint(log.PrescriptionID)"
                >
                  Reprint
                </button>
              </li>
          ...
          <ul v-if="orders.length > 0">
            ...
            >
              ...
              <div class="pool-column">
                ...
                <b v-else-if="order.DeliveryID == 7 && order.PaymentMethod == 0"
                  >UPS</b
                >
                <b v-else-if="order.DeliveryID == 7 && order.PaymentMethod != 0"
                  >UPS COD</b
              ...
              <div class="pool-actions">
                <button
                  :disabled="
                    order.UserID == userInfo.esa_user_id || userInfo.role != 20
                  "
                ...
                <button
                  :disabled="order.UserID == 0"
                  @click="release(false, order.DispenserPoolID)"
    ...
    <section v-if="tray.length == 0" class="card">
      <div class="card-header pool-list-layout">
        <div class="pool-column">Dispenser</div>
        <div class="pool-column">Available Orders</div>
        <div class="pool-actions">Tools</div>
      </div>
      ...
      <div class="card-body">
        <ul>
            ...
            <div class="pool-actions">
              ...
              <button
                ...
                :disabled="
                  ...
                  count.rml == 0 ||
                  (!trayCompany.includes('rml') && trayCompany.length > 0)
                "
                @click="allocate(false, 'rml', false)"
                class="btn btnSize02 tertiaryBtn ml-20"
              >
              ...
              <button
                ...
                :disabled="
                  (userInfo.role != 20 && userInfo.role != 19) ||
                  count.dpd == 0 ||
                  (!trayCompany.includes('dpd') && trayCompany.length > 0)
                "
                @click="allocate(false, 'dpd', false)"
                class="btn btnSize02 tertiaryBtn ml-20"
              >
                DPD <b>({{ count.dpd }})</b>
              </button>
              ...
              <button
                ...
                title="Take over all available UPS orders"
                :disabled="
                  (userInfo.role != 20 && userInfo.role != 19) ||
                  count.ups == 0 ||
                  (!trayCompany.includes('ups') && trayCompany.length > 0)
                "
                @click="allocate(false, 'ups', false)"
                class="btn btnSize02 tertiaryBtn ml-20"
              >
                UPS <b>({{ count.ups }})</b>
              </button>
              ...
              <button
                v-if="appInfo.delivery_companies.includes('ups')"
                title="Take over all available UPS COD orders"
                :disabled="
                  (userInfo.role != 20 && userInfo.role != 19) ||
                  count.upscod == 0 ||
                  (!trayCompany.includes('upscod') && trayCompany.length > 0)
                "
                @click="allocate(false, 'upscod', false)"
                class="btn btnSize02 tertiaryBtn ml-20"
              >
                UPS COD <b>({{ count.upscod }})</b>
              </button>

              <button
                v-if="appInfo.delivery_companies.includes('dhl')"
                title="Take over all available DHL orders"
                :disabled="
                  (userInfo.role != 20 && userInfo.role != 19) ||
                  count.dhl == 0 ||
                  (!trayCompany.includes('dhl') && trayCompany.length > 0)
                "
                @click="allocate(false, 'dhl', false)"
                class="btn btnSize02 tertiaryBtn ml-20"
              >
                DHL <b>({{ count.dhl }})</b>
              </button>
          ...
          >
            ...
            <div class="pool-actions">
              <button
                :title="'Take over orders assigned to ' + dispenser.name"
                :disabled="dispenser.count == 0"
                @click="allocate(dispenser.esa_user_id, false, false)"
                v-if="dispenser.id != userInfo.id"
                class="btn btnSize02 tertiaryBtn mr-20"
              >
                TAKE OVER
              </button>

              <button
                :title="'Release orders assigned to ' + dispenser.name"
                :disabled="dispenser.count == 0"
                @click="release(dispenser.esa_user_id, false)"
                class="btn btnSize02 tertiaryBtn"
              >
                RELEASE
              </button>
...
export default {
  ...
  computed: {
    showListText() {
      return this.idsVisible
        ? "Hide List of Order ID's"
        : "Show List of Order ID's";
    },
    availableCount() {
    ...
    count() {
      ...
      this.orders.forEach((item) => {
        ...
        if (item.ClientID == 51 && item.UserID == 0 && item.JVM == 0) {
          response.eveadam++;
        }
        if (item.UserID == 0 && item.JVM == 1 && item.ClientID == 51) {
          response.jvm++;
        }
      });
  ...
  methods: {
    getPendingOrders() {
      axios
        .post('/prescription-pool/orders-list', { ids: this.trayIds })
        .then((response) => {
    ...
    getDispensers() {
      axios
        .get('/prescription-pool/dispensers')
        .then((response) => {
    ...
    allocate(userID = false, deliveryID = false, orderID = false) {
      axios
        .post('/prescription-pool/allocate', {
          userID: userID,
    ...
    releaseRequest(userID = false, dispenserPoolID = false, all = false) {
      axios
        .post('/prescription-pool/release', {
          userID: userID,
