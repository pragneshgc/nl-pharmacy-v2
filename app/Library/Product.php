...
class Product
{
    ...
    public function formatProducts($data)
    {
        ...
        foreach ($data as $value) {
            ...
            if (isset($value->Price)) {
                $value->Price = $currency . ' ' . number_format((float)$value->Price, 2, '.', ',');
            }
     ...
     * @param int $id
     */
    public function getAlternativeNames(int $id): Collection
    {
        return DB::table('ProductNameAlternative AS pna')
            ->select(['pna.ProductNameAlternativeID', 'pna.ProductCodeID', 'c.CompanyName', 'pna.AlternativeName', 'u.name AS Name', 'u.surname AS Surname'])
            ->selectRaw("DATE_FORMAT(pna.CreatedAt, '%d/%c/%Y %H:%i') AS CreatedAt")
    ...
    /**
     * Approve an alternative unit for a product
     *
     * @param Request $request
     */
    ...
    {           
        ...
        $array = Excel::toCollection([], $request->file('file'))[0];
