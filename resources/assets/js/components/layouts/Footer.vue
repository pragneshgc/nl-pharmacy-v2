    ...
    <div class="footer" v-if="user.info.role != 4">
        <div class="tray-drawer" :class="{ 'open': show.tray }">
            <div class="tray-toolbar">
                <select v-if="[30, 35, 50, 60].includes(this.user.info.role)" v-model="user.selected" class="table-dropdown"
...
export default {
    ...
    methods: {
        ...
        deleteTrayByPrescription(id, skip = false) {
            let trayId = this.getTrayId(id);
            console.log('id', id);
            console.log('tray id', trayId);
            this.deleteTray(trayId);
        ...
        xmlDownload() {
            //axios.get(`/order/${this.prescription.PrescriptionID}/xml?token=${this.user.info.token}`);
            this.downloadURI(`/order/${this.prescription.PrescriptionID}/xml?token=${this.user.info.token}`, `${this.prescription.PrescriptionID}.xml`);
        },
        closeDuplicateModal() {
