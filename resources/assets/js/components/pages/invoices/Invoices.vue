            ...
            <div class="card-body">
                <div class="invoice-search">
                    <TableComponentSearch data-url="/invoices" column-class="col-lg-12" table-title="Invoices"
                        redirect-name="invoice" redirect-id="InvoiceID" :hidden-columns="[
                            'InvoiceID',
                            'ClientID',
                            'SequenceID',
                            'TradingName',
...
export default {
    ...
    methods: {
        downloadInvoice(item) {
            let url = `/invoice/${item['Invoice #']}/download?token=${this.userInfo.token}`;
            console.log(url);
            this.downloadFileViaURL(url, `${item['Invoice #']}.pdf`);
        },
    },
};
</script>
