...
class Order
{
    ...
    public function order($patientDetails = false, $trayStatus = false, $csv = false)
    {
        ...
        $offsetFormatted = $this->timezoneOffset();
        ...
        if ($trayStatus) {
            $data = $data->selectRaw("DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(Prescription.CreatedDate), '+00:00', '" . $offsetFormatted . "'), '%e %b %Y %H:%i') AS 'Received Date'");

            if ($user->role == 20 || $user->role == 19) {
                $data = $data->selectRaw("Prescription.Exemption, Prescription.PaymentMethod, Prescription.UPSAccessPointAddress"); //checkboxes are not used here so no need
            } else {
                $data = $data->selectRaw("Prescription.Exemption, Prescription.PaymentMethod, COALESCE((SELECT Status FROM Tray WHERE Tray.PrescriptionID = Prescription.PrescriptionID AND Status = 1 LIMIT 1),0) AS disabled, Prescription.UPSAccessPointAddress");
            }
        } else {
            $data  = $data->selectRaw("DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(Prescription.CreatedDate), '+00:00', '" . $offsetFormatted . "'), '%e %b %Y %H:%i') AS 'Received Date'")
                ->selectRaw("DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(Prescription.UpdatedDate), '+00:00', '" . $offsetFormatted . "'), '%e %b %Y %H:%i') AS 'Processed Date'");
        }

        if ($csv) {
            $data = $data->selectRaw("CONCAT(COALESCE(Prescription.DAddress1, ''), ' ',
    ...
    public function register($csv = false)
    {
        ...
        $data = DB::table('Prescription')
            ->selectRaw("Prescription.PrescriptionID, Prescription.ReferenceNumber")
            ->selectRaw("DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(Prescription.CreatedDate), '+00:00', '" . $offsetFormatted . "'), '%e %b %Y %H:%i') AS 'Date Recieved'")
            ->selectRaw("DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(Prescription.UpdatedDate), '+00:00', '" . $offsetFormatted . "'), '%e %b %Y %H:%i') AS 'Date Supplied'")
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'status-extended':
                        $data = $data->whereRaw('(Prescription.Status = ? OR Prescription.SubStatus = ?)', [$value, $value]);
                        break;
                    case 'statuses':
    ...
    public function displayUnknownSearchParameters($data, $items, $filters, $target, $caller = 'search')
    {
        ...
        if (isset($filters->{$target})) {
            ...
            if (count($data->items()) > 0 && count($searchedIds) > 0 && $searchedIds[0] != '') {
    ...
    public function getPrintDetails($id): mixed
    {
        ...
        CAST(SUM(IF((pai.PAIID || pwl.PWLID && p.CountryCode!=1), 1, 0)) AS UNSIGNED) AS 'ProductAdditionalInformation',
        ...
        ")
            ...
            ->where('p.PrescriptionID', $id)->first();
    }
    ...
    public function insertLabel($id)
    {
        $products = DB::table('Product')
            ->select('Product.*', 'ProductCode.Name', 'ProductCode.Units', 'ProductCode.Quantity AS ProductCodeQuantity')
            ->leftJoin('ProductCode', 'ProductCode.Code', '=', 'Product.Code')
            ->where('PrescriptionID', $id)
            ->get();
    ...
    public function prescriptionView($id)
    {
        ...
        //$signature = URL::to('/') . "/doctors/" . $prescription->DoctorID . "/signature?prescription=" . $prescription->PrescriptionID;
        $signature_src = base64Signature($prescription->DoctorID, $prescription->PrescriptionID);
        /* if (App::environment(['local', ['stagging']])) {
            $fileContent = getFileContents($signature);
        } else {
            $fileContent = file_get_contents($signature);
        }
        $base64Image = base64_encode($fileContent);
        $signature_src = "data:image/png;base64," . str_replace("\n", "", $base64Image); */

        return View::make($prescriptionPath, compact('prescription', 'genders', 'doctorTypes', 'products', 'pharmacy', 'testkits', 'params', 'showCautionaryAdvice', 'signature_src'))->render();
    }
    ...
    public function redeliver(int $id): int
    {
        ...
        //    ->select(['p.TrackingCode', 'p.DeliveryID', 'p.UpdatedDate'])
        //    ->where('PrescriptionID', $id)->first();
        ...
        if ($prescription) {
            ...
            if (!$existingCustomer) {
                $customerId = DB::table('CustomerPrescriptions')->insertGetId([
                    ...
                    'ModifiedDate' => now()->timestamp,
                    'AccessedDate' => now()->timestamp,
    ...
    {
        //the below is a very quick hack to handle the timezone issue we got since the server is in UK and the app is in NL
        //should be an easy fix when using models instead of query builder
