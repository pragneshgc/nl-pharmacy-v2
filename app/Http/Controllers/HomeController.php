...
class HomeController extends Controller
{
    ...
    {
        $deliveryCompanies = config('app.delivery_companies');
        ...
        if (!in_array('dpd', $deliveryCompanies)) {
            unset($statistics['dpd']);
        }

        if (!in_array('ups', $deliveryCompanies)) {
            unset($statistics['ups']);
        }

        if (!in_array('dhl', $deliveryCompanies)) {
            unset($statistics['dhl']);
        }

        if (!in_array('rml', $deliveryCompanies)) {
            unset($statistics['rml']);
        }
                ...
                switch ($status->Status) {
                    ...
                    case '7':
                        ...
                        if (in_array($role, [20, 40, 50, 60])) {
                            if ($status->DeliveryID == 4 && in_array('dpd', $deliveryCompanies)) {
                                $statistics['dpd'] += (int) $status->Count;
                            } else if ($status->DeliveryID == 7 && in_array('ups', $deliveryCompanies)) {
                                $statistics['ups'] += (int) $status->Count;
                            } else if ($status->DeliveryID == 10 && in_array('dhl', $deliveryCompanies)) {
                                $statistics['dhl'] += (int) $status->Count;
                            } else if ($status->DeliveryID == 5 && in_array('rml', $deliveryCompanies)) {
        ...
        if (in_array($role, [29, 30, 35])) {
            ...
            if (in_array('dpd', $deliveryCompanies)) {
                $total += (int) $statistics['dpd'];
            }

            if (in_array('ups', $deliveryCompanies)) {
                $total += (int) $statistics['ups'];
            }

            if (in_array('dhl', $deliveryCompanies)) {
                $total += (int) $statistics['dhl'];
            }

            if (in_array('rml', $deliveryCompanies)) {
                $total += (int) $statistics['rml'];
            }
        }
